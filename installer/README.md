...
The wizard lets the user choose **which Navisworks versions** (2024 / 2025 / 2026 / 2027) and **which flavour**
...
> (2024 = `Api 21.x`, 2025 = `22.x`, 2026 = `23.x`, 2027 = `24.x`); loading a mismatched DLL fails with
> `PLUGIN_LOAD_07: invalid referenced Navisworks Api version`. So the bundle carries a **separate DLL
> per version** in its own year folder (`…\BIMCamel.bundle\2024\`, `\2025\`, `\2026\`, `\2027\`), each with its
...
2. **Navisworks check page** — instead of decision pop-ups, an early page lists which supported
   Navisworks (Manage / Simulate, 2024–2027) it found and where, and **pre-selects those versions**
...
This:
1. Builds the plugin in **Release once per Navisworks version** (2024/2025/2026/2027), each against that
...
3. Compiles `BIMCamel_Setup.exe` into `installer\output\` (all four year DLLs are staged from NuGet).
...
- **Manual fallback (zero tooling):** the plug-in is just a folder. A ready-to-copy `BIMCamel.bundle`
  skeleton lives in `..\dist\` — drop your per-version DLLs into its `2024\` / `2025\` / `2026\` / `2027\`
